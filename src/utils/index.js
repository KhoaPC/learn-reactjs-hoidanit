...
export function formatDate(format, input = new Date()) {
  let seperate1;
  let seperate2;
  const rgexD_M_Y1 =
  ...
  if (
    rgexD_M_Y1.test(input) ||
    rgexD_M_Y2.test(input) ||
    rgexD_M_Y3.test(input) ||
    rgexD_M_Y4.test(input)
  ) {
    if (input.includes("/")) input = input.split("/").reverse().join("-");
    else if (input.includes(",")) input = input.split(",").reverse().join("-");
    else if (input.includes(".")) input = input.split(".").reverse().join("-");
    else if (input.includes("-")) input = input.split("-").reverse().join("-");
    else if (input.includes(" ")) input = input.split(" ").reverse().join("-");
  } else {
    if (input.includes("/")) input = input.split("/").join("-");
    else if (input.includes(",")) input = input.split(",").join("-");
    else if (input.includes(".")) input = input.split(".").join("-");
    else if (input.includes("-")) input = input.split("-").join("-");
    else if (input.includes(" ")) input = input.split(" ").join("-");
  }
  ...
  for (let i = 0; i < mLength; i++) {
    if (format === CONST_GET_M[i]) index = arr1.indexOf("M");
  }
  for (let i = 0; i < dLength; i++) {
    if (format === CONST_GET_D[i]) index = arr1.indexOf("D");
  }
...
export function getWeekdayName(input = new Date()) {
  const time = new Date(input);
...
export function getCurrentTime_ISOformat() {
  ...
  return `${help(year)}-${help(month)}-${date}T${help(hours)}:${help(
    minutes
  )}:${help(second)}`;
...
export function getTotalDaysDifferent(fromDate, toDate) {
...
export function getDateDifferent(input) {
  const {
    day: inputD,
    month: inputM,
    year: inputY,
  } = getDateParts(formatDate("YYYY-MM-DD", input));
  ...
  if (inputM >= todayM) diffMonths = inputM - todayM;
  else {
  ...
  } else if (inputD - todayD >= 0) diffDays = inputD - todayD;
  else {
    CONST_MONTH_31DAY.forEach((item) => {
      if (todayM == item) {
        diffDays += 31;
  ...
  const diffDaysTotal = getTotalDaysDifferent(
    getCurrentTime_ISOformat(),
    `${inputY}-${inputM}-${inputD}`
  );
